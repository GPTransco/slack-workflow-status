...
async function main(){
  ...
  const commit_message: string = "<"+workflow_run.data.head_commit.message+"|#"+workflow_run.data.head_commit.message+">"

  // Example: Success: AnthonyKinson's `push` on `master` for pull_request
  let status_string: string = workflow_msg+" "+actor+"'s **"+commit_message+"** `"+event+"` on `"+branch_url+"`\n"
